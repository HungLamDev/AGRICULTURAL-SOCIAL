...
const authCtrl = {
  register: async (req, res) => {
    try {
      ...
      if(user_name) return res.status(404).json({ msg: "Tên người dùng đã tồn tại!" });

      const user_email = await User.findOne({user_email: newUserName})
      if(user_email) return res.status(404).json({ msg: "Email này đã tồn tại!" });
      
      if(password.length < 6) 
      return res.status(404).json({ msg: "Mật khẩu phải có ít nhất 6 ký tự trở lên!" });
