    ...
    <div className='auth_page'>
      <form onSubmit={handleSubmit}>
        ...
        <div className='row justify-content-between mx-0 mb-1'>
          <label htmlFor='nam'>
              Nam: <input type='radio' id='nam' name='gender' value='nam' defaultChecked onChange={handleChangeInput}/>
          </label>
          <label htmlFor='nu'>
              Nữ: <input type='radio' id='nu' name='gender' value='nu' defaultChecked onChange={handleChangeInput}/>
          </label>
          <label htmlFor='khac'>
              Khác: <input type='radio' id='khac' name='gender' value='khac' defaultChecked onChange={handleChangeInput}/>
          </label>
        </div>

        <button type="submit" className="btn btn-dark w-100" style={{ backgroundColor: 'red', borderColor: 'red' }}
        >
          Đăng Ký
        </button>

        <p className='my-2 text-center'>
            Bạn đã có tài khoản? < Link to="/" style={{color: "crimson"}}>Đăng nhập ngay</Link>
        </p>
      </form>
    </div>
  )
