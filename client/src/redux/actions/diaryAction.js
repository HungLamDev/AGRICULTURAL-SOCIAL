...
export const createDiary =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      console.log(res);
      dispatch({
        type: DIARYTYPES.CREATE_DIARY,
        payload: { ...res.data.newDiary, user: auth.user },
      });
    } catch (err) {
      dispatch({
...
export const updateDiary =
  ({ content, images, arrIdPost, auth, diary }) =>
  async (dispatch) => {
...
export const saveDiary =
  ...
  async (dispatch) => {
    const newUser = {
      ...auth.user,
      savedDiary: [...auth.user.savedDiary, diary._id],
    };
...
export const unSaveDiary =
  ({ diary, auth }) =>
  async (dispatch) => {
    const newUser = {
      ...auth.user,
      savedDiary: auth.user.savedDiary.filter((id) => id !== diary._id),
    };
    dispatch({ type: GLOBALTYPES.AUTH, payload: { ...auth, user: newUser } });
