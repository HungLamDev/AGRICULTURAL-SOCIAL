            ...
            <ul className="navbar-nav flex-row">
                  {
                        navLink.map((link,index)=>(
                              <li className={`nav-item px-2  ${isActive(link.path)}`} key={index}>
                                    ...
                                    style={isActive(link.path) ? { color: 'gray' } : { color: '#66CC66' }}
                                    >
                                    {link.icon}
                                    </span>
                  ...
                  <li className="nav-item dropdown">
                        <span className="nav-link dropdown-toggle pb-3" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                              <Avatar src={auth.user.avatar} theme={theme} size="medium-avatar"/>
                        </span>
                        <div className="dropdown-menu" aria-labelledby="navbarDropdown">
                              <Link className="dropdown-item" to={`/profile/${auth.user._id}`}>Thông tin cá nhân</Link>
