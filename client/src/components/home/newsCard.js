  ...
  return (
    ...
    >
      <div className="news_card">
        <Avatar src={posts.user.avatar} size="big-avatar" />
        <div className="news_container">
          <div className="card_header">
            <div>
              <small className="card_username">{posts.user.username}</small>
          ...
          <div className="card_footer">
            ...
            <small>{posts.like.length}</small>
            <i
              className="fa-solid fa-comment"
              style={{ marginLeft: "10px" }}
            ></i>
            <small>{posts.comments.length}</small>
          </div>
        </div>
      </div>
    </Link>
